...
export default class SearchFlowComponent extends Component {
  ...
  get filters() {
    console.log('===== FILTERS GETTER CALLED =====');
    console.log('this.args.query:', this.args.query);
    console.log('this.args.parameters:', this.args.parameters);
    console.log('this._manualFilters:', this._manualFilters);
    console.log('this.queryGeneratedByComponent:', this.queryGeneratedByComponent);
    
    // If manually set filters exist, use those
    if (this._manualFilters) {
      console.log('Returning manual filters');
      return this._manualFilters;
    }

    // Otherwise, compute from query
    if (this.queryGeneratedByComponent) {
      console.log('Query generated by component, returning empty or manual filters');
      return this._manualFilters || A([]);
    }
    
    if (!this.args.query || !this.args.parameters) {
      console.log('No query or parameters, returning empty array');
      return A([]);
    }
  ...
  get availableParameters() {
    console.log('===== AVAILABLE PARAMETERS GETTER CALLED =====');
    console.log('this.args.parameters:', this.args.parameters);
    console.log('this.filters:', this.filters);
    
    const result = this.args.parameters?.reject(parameter => {
      const shouldReject = !parameter.name || !parameter.title || (parameter.allowMultiple === false && this.filters.find(filter => {
        return filter?.parameter?.name === parameter.name;
      }));
      if (shouldReject) {
        console.log('Rejecting parameter:', parameter);
      }
      return shouldReject;
    }) || A([]);
    
    console.log('availableParameters result:', result);
    console.log('availableParameters length:', result.length);
    return result;
  }
