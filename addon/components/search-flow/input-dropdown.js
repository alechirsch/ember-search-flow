...
class InputDropdownComponent extends Component {
	...
	@tracked activeOptionIndex = 0; // Track the active option index
	@tracked keepDropdownOpen = false; // Keep dropdown open during click
	inputElement = null;
	...
	constructor() {
		...
		schedule('afterRender', this, () => {
			if ((!this.value && this.args.filter) || this.args.isParameterSelection) {
			this.focusInput();
			}
			this.setInputWidth();
		});
	...
	get availableOptions() {
		...
		// Also keep open if keepDropdownOpen flag is set (during click handling)
		if (!this.isFocused && !this.args.isParameterSelection && !this.keepDropdownOpen) {
			return A([]);
		}
	...
	get activeOption() {
		return this.availableOptions[this.validActiveOptionIndex];
	}

	@action 
	updateValue(event) {
		this.value = event.target.value;
		// Reset to first option when typing
	...
	setActiveOption(option) {
		// Use the index property that's set on each option
		if (option && typeof option.index === 'number') {
			this.activeOptionIndex = option.index;
		}
	}

	@action
	wrapperMouseDown(_event) {
		// Keep dropdown open during click handling for ANY click inside the wrapper
		this.keepDropdownOpen = true;
		
		// Clear the flag after click has time to fire
		setTimeout(() => {
	...
	selectOption(clickedOption) {
		...
		// Safety check - if no active option, return early
		if (!activeOption) {
			return;
		}
		...
		// Clear keepDropdownOpen so the blur action proceeds
		this.keepDropdownOpen = false;
		this.blurInput();
	...
	inputKeyDown(event) {
		if (event.which === 38) { // Up
			...
			let currentIndex = this.validActiveOptionIndex;
			if (currentIndex > 0) {
			this.activeOptionIndex = currentIndex - 1;
			}
		}
	...
	inputBlurred(event) {
		...
		// This allows clicking on options (which have tabindex="-1") without closing the dropdown
		if (event.relatedTarget && 
			(event.relatedTarget.closest('.search-flow_dropdown__bg') || 
				event.relatedTarget.closest('.search-flow_dropdown-option'))) {
			return;
		}
		...
		setTimeout(() => {
			...
			// Check if we should keep open (e.g. mousedown handling)
